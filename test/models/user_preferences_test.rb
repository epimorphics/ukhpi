...
class UserPreferencesTest < ActiveSupport::TestCase
  it 'permits access to whitelisted parameters' do
    up = UserPreferences.new(ActionController::Parameters.new('region' => 'foo'))
  ...
  it 'provides a default value for region' do
    up = UserPreferences.new(ActionController::Parameters.new('region' => ''))
  ...
  it 'accepts valid dates' do
    up = UserPreferences.new(
      ActionController::Parameters.new('from' => '2015-01-01', 'to' => '2016-01-01')
    )
  ...
  it 'rejects invalid dates' do
    lambda {
      UserPreferences.new(ActionController::Parameters.new('from' => 'century of the fruitbat'))
    }.must_raise ArgumentError
  end

  it 'provides a default date' do
    up = UserPreferences.new(
      ActionController::Parameters.new('region' => 'foo', 'from' => '', 'to' => '')
    )
    up.from.must_be_kind_of Date
    up.to.must_be_kind_of Date
  end
  ...
  it 'provides a default selection for aspect indicators' do
    up = UserPreferences.new(ActionController::Parameters.new('ai' => ''))
  ...
  it 'provides a default selection for aspect categories' do
    up = UserPreferences.new(ActionController::Parameters.new('ac' => ''))
  ...
  it 'reports selected aspect indicators as an array' do
    up = UserPreferences.new(ActionController::Parameters.new('ai' => %w[foo bar]))
  ...
  it 'decodes selected aspect indicators as an array' do
    up = UserPreferences.new(ActionController::Parameters.new('ai' => 'foo,bar'))
    up.aspect_indicators.must_be_kind_of Array
    up.aspect_indicators.first.must_equal :foo
    up.aspect_indicators.second.must_equal :bar
    up.aspect_indicators.length.must_equal 2
  ...
  it 'wraps a single aspect indicator as an array' do
    up = UserPreferences.new(ActionController::Parameters.new('ai' => 'foo'))
    up.aspect_indicators.must_be_kind_of Array
    up.aspect_indicators.first.must_equal :foo
  ...
  it 'can encode parameters as a URL search string' do
    UserPreferences
      .new(ActionController::Parameters.new('region' => 'foo', 'from' => '2016-04'))
      .as_search_string
  ...
  it 'can encode array-valued parameters as a search string' do
    UserPreferences
      .new(ActionController::Parameters.new('ai' => %w[foo bar]))
      .as_search_string
  ...
  it 'should summarise itself' do
    ...
    up1 = UserPreferences.new(
      ActionController::Parameters.new('region' => 'foo', 'to' => '2016-01-01')
    )
  ...
  it 'should correctly summarise a URI valued location' do
    up = UserPreferences.new(
      ActionController::Parameters.new('region' => 'http://landregistry.data.gov.uk/id/region/great-britain')
    )
