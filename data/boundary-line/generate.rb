...
def create_index(filename, index = {})
...
def point_to_lat_long(point)
  ...
  lat_long_point = RGeo::Feature.cast(bng_point, type: RGeo::Feature::Point, factory: LATLONG_PROJECTION, project: true) # rubocop:disable Metrics/LineLength
...
def load_json
  File.exist?('fc.json') && JSON.parse(File.open('fc.json'))
end
